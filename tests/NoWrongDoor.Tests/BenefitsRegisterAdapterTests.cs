...
public class BenefitsRegisterAdapterTests
{
    ...
    public void Setup()
    {
        var client = new HttpClient { BaseAddress = new Uri("http://127.0.0.1:8082/") };
    ...
    public async Task SearchAsync_ReturnsOkWithRealRecords_NotMalformed()
    {
        ...
        Assert.That(result.Status, Is.EqualTo(SourceStatus.Ok),
    ...
    public async Task GetByRefAsync_UnknownRef_ReturnsEmpty()
    {
        ...
        Assert.That(result.Status, Is.EqualTo(SourceStatus.Empty));
    }
    ...
    public async Task GetByRefAsync_RealRefFromSearch_ReturnsOk()
    {
        ...
        Assert.That(searchResult.Status, Is.EqualTo(SourceStatus.Ok));
        ...
        Assert.That(result.Status, Is.EqualTo(SourceStatus.Ok),
            $"Expected Ok but got {result.Status}. Note: {result.Note}");
        Assert.That(result.Data!.SourceId, Is.EqualTo(firstRef));
    }
    ...
    public async Task SearchAsync_RepeatedCalls_EventuallyHitsA500AndReturnsUnavailable_OrOk()
    {
        ...
        Assert.That(statuses, Has.All.Matches<SourceStatus>(
            s => s == SourceStatus.Ok || s == SourceStatus.Unavailable || s == SourceStatus.Malformed));

        Console.WriteLine($"Statuses over 20 calls: {string.Join(", ", statuses)}");
    }
}